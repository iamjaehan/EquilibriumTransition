...
% u_before = [0.2, -0.1] * 0.5;   % u for t < t_switch
% u_after  = [-0.1, 0.2] * 0.5;   % u for t >= t_switch
u_phase1 = [-0.5,  0.0];    % u for t < t_switch1
u_phase2 = [-0.5, -0.2];    % u for t_switch1 <= t < t_switch2
u_phase3 = [ 0.0, 0.0];    % u for t >= t_switch2
t_switch1 = T / 3;
t_switch2 = 2 * T / 3;
...
u_fn = @(t, x) select_u(t, u_phase1, u_phase2, u_phase3, t_switch1, t_switch2);
...
badSteps = find(any(~isfinite(Xtraj), 2));
if ~isempty(badSteps)
    fprintf('NaN/Inf at steps: %s (t = %s)\n', ...
        num2str(badSteps.'), num2str(tgrid(badSteps).'));
end
...
P = tern2xy(Xtraj);
k_sw1 = find(tgrid >= t_switch1, 1);
k_sw2 = find(tgrid >= t_switch2, 1);
u_phase2_tilde = [u_phase2(1), u_phase2(2), 0.0];
u_phase3_tilde = [u_phase3(1), u_phase3(2), 0.0];
Vbg_phase2 = [];  Vbg_phase3 = [];  % computed after Xbg/tri are ready (below)
...
Vbg_phase2 = 0.5 * sum(Xbg.^2, 2) + u_potential_gain * (Xbg * u_phase2_tilde.');
Vbg_phase3 = 0.5 * sum(Xbg.^2, 2) + u_potential_gain * (Xbg * u_phase3_tilde.');
XYbg = tern2xy(Xbg);
...
plot(P(1:k_sw1,      1), P(1:k_sw1,      2), 'b-', 'LineWidth', 2.2);   % phase 1
plot(P(k_sw1:k_sw2,  1), P(k_sw1:k_sw2,  2), 'r-', 'LineWidth', 2.2);   % phase 2
plot(P(k_sw2:end,    1), P(k_sw2:end,    2), 'm-', 'LineWidth', 2.2);   % phase 3
plot(P(1,1), P(1,2), 'go', 'MarkerFaceColor', 'g', 'MarkerSize', 8);
plot(P(end,1), P(end,2), 'ko', 'MarkerFaceColor', 'k', 'MarkerSize', 8);
...
% Dynamic background (starts with u_before, updates at t_switch)
hBgPatch = patch('Faces', tri, 'Vertices', XYbg, ...
    'FaceVertexCData', Vbg, 'FaceColor', 'interp', ...
    'EdgeColor', 'none', 'FaceAlpha', 0.58);
colormap(parula);
hBgOverlay = gobjects(0,1);
if show_contour
    tr_d = triangulation(tri, XYbg);
    [~, hCd] = tricontour(tr_d, Vbg, 10);
    set(hCd, 'Color', [0.1 0.1 0.1], 'LineWidth', 0.8);
    hBgOverlay = [hBgOverlay; hCd(:)];
end
if show_grad_quiver
    Gbgd = -(Xbg + u_potential_gain * ones(size(Xbg,1),1) * u0_tilde);
    Gbgd = Gbgd - mean(Gbgd, 2) .* ones(size(Gbgd));
    XYbg2d = tern2xy(proj_simplex_rows(Xbg + quiver_scale * Gbgd));
    Dxyd = XYbg2d - XYbg;
    hQd = quiver(XYbg(qidx,1), XYbg(qidx,2), Dxyd(qidx,1), Dxyd(qidx,2), 0, ...
        'Color', [0.2 0.2 0.2], 'LineWidth', 0.7, 'MaxHeadSize', 1.2);
    hBgOverlay = [hBgOverlay; hQd];
end
...
for k = 2:size(P,1)
    if k == k_sw1 || k == k_sw2
        ...
        u_tilde_new = u_phase2_tilde * (k == k_sw1) + u_phase3_tilde * (k == k_sw2);
        set(hBgPatch, 'FaceVertexCData', Vbg_new);
        delete(hBgOverlay); hBgOverlay = gobjects(0,1);
        if show_contour
            tr_d = triangulation(tri, XYbg);
            [~, hCd] = tricontour(tr_d, Vbg_new, 10);
            set(hCd, 'Color', [0.1 0.1 0.1], 'LineWidth', 0.8);
            hBgOverlay = [hBgOverlay; hCd(:)];
        end
        if show_grad_quiver
            Gbgd = -(Xbg + u_potential_gain * ones(size(Xbg,1),1) * u_tilde_new);
            Gbgd = Gbgd - mean(Gbgd, 2) .* ones(size(Gbgd));
            XYbg2d = tern2xy(proj_simplex_rows(Xbg + quiver_scale * Gbgd));
            Dxyd = XYbg2d - XYbg;
            hQd = quiver(XYbg(qidx,1), XYbg(qidx,2), Dxyd(qidx,1), Dxyd(qidx,2), 0, ...
...
function u = select_u(t, u_phase1, u_phase2, u_phase3, t_switch1, t_switch2)
if t < t_switch1
    u = u_phase1;
elseif t < t_switch2
    u = u_phase2;
